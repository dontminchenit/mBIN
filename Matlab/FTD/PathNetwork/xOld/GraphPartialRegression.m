...
winBase = fullfile('D:','Min','Dropbox (Personal)','Research','Projects','FTDAnalysis');
macBase = fullfile('/Users','Min','Dropbox (Personal)','Research','Projects','FTDAnalysis');
currBase = macBase;

inT = readtable(fullfile(currBase,'Data','Working Network Path Sheet 1_29_21.xlsx'));
...
NumSamples=sum(~isnan(Y));

Design = zeros(NumSamples,2);
Design(:,1) = 1;
Design(:,2) =Y(~isnan(Y));
...
%mdl = fitlm(gmTvals(:,2:end),gmTvals(:,1))
