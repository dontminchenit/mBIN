function [TauNetworkBurdenCorr,TDPNetworkBurdenCorr, MarkerNames,MarkerSizeTau,MarkerSizeTDP]=analyzeGraphsSubGroup(inTable,TableIdxCore,removeRegions,baseSaveDir,GMWMOffset,ExistingMarkers,TauSubGroupName,TDPSubGroupName)
...
if(strcmp('All',TauSubGroupName))
    Table_Tau=inTable(inTable.Tau2TDP1==2 & inTable.Include ==1, TableIdxCore);
else
    Table_Tau=inTable(inTable.Tau2TDP1==2 & inTable.Include ==1 & ~strcmp(inTable.Subgroup,TauSubGroupName),TableIdxCore);
end

if(strcmp('All',TDPSubGroupName))
    Table_TDP=inTable(inTable.Tau2TDP1==1 & inTable.Include ==1, TableIdxCore);
else
    Table_TDP=inTable(inTable.Tau2TDP1==1 & inTable.Include ==1 & ~strcmp(inTable.FTLDTDP,TDPSubGroupName),TableIdxCore);
end
...
for Tau1TDP2 = 1:2
    ...
    if(plotCorrFlag)
        H=figure(2);
        clf
        %corrplot(TableCurrentLogVals,'varNames',baseNames)
        CorrAdjMtxDisp = CorrAdjMtx;
        CorrAdjMtxDisp(CorrAdjMtxCount<=3)=2;
        CorrAdjMtxDisp(isnan(CorrAdjMtx))=2;
        h=imagesc(CorrAdjMtxDisp')
        % Set nan values to transparent: 
        %set(h,'alphadata',~isnan(CorrAdjMtx'))
        ...
        if(Tau1TDP2==1)
            saveName = [TauSubGroupName '_tau_MTX3.png'];
            %title(TauSubGroupName)
        else
            saveName = [TDPSubGroupName '_tdp_MTX3.png'];
            %title(TDPSubGroupName)
        end
    ...
    if(plotCorrFlag)
        ...
        xtickangle(90)
        if(Tau1TDP2==1)
            saveName = [TauSubGroupName '_tau_Count.png'];
            title(TauSubGroupName)
            savePrefix ='tau_'
        else
            saveName = [TDPSubGroupName '_tdp_Count.png'];
            title(TDPSubGroupName)
            savePrefix ='tdp_'
        end
    ...
    if(Tau1TDP2 == 1)
        ...
        if(~exist('ExistingMarkers','var') || isempty(ExistingMarkers))
...
if(plotGraphsFlag)
    ...
    for TauTDPcmp = 3:4
        if TauTDPcmp==1
            plotMtx = TauSigLower;
            MarkerSize=MarkerSizeTau;
            saveName = [TauSubGroupName '_vs_' TDPSubGroupName '_tauLowerThanTDP_comp'];
            Tau1TDP2=2;
        elseif TauTDPcmp==2
            plotMtx = TDPSigLower;
            MarkerSize=MarkerSizeTDP;
            saveName = [TauSubGroupName '_vs_' TDPSubGroupName '_TDPLowerThanTau_comp'];
            Tau1TDP2=2;
        elseif TauTDPcmp==3
            plotMtx = TDPSigLower;
            MarkerSize=MarkerSizeTau;
            saveName = [TauSubGroupName '_vs_' TDPSubGroupName '_tauGreaterThanTDP_comp'];
            Tau1TDP2=1;
        elseif TauTDPcmp==4
            plotMtx = TauSigLower;
            MarkerSize=MarkerSizeTDP;
            saveName = [TauSubGroupName '_vs_' TDPSubGroupName '_TDPGreaterThanTau_comp'];
            Tau1TDP2=2;
        end
