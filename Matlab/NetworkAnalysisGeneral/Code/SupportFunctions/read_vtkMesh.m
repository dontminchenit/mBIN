function labelmesh =read_vtkMesh(filename)
...
while ~feof(fid)
    ...
    elseif (strfind(str,'polygons'))
        ...
        labelmesh.triangles = triangles+1; % in matlab, first index is 1;
        labelmesh.ntriangles = size(labelmesh.triangles,1);
        clear t;
    ...
    elseif (strfind(str,'cells'))
        ...
        labelmesh.triangles = t+1; % in matlab, first index is 1;
        labelmesh.ntriangles = size(t,1);
        clear t;
    ...
    elseif (strfind(str,'point_data'))
        ...
        while(still_in_point_data)
            ...
            if (strcmp(attribute,'scalars'))
                ...
                % fill attribute body ---------------------
                if strcmp(at.type,'short') || strcmp(at.type,'unsigned_short')
                    [num count]=fscanf(fid,'%d');
            ...
            elseif (strcmp(attribute,'vectors'))
               % continue;
                ...
                at = AttributeType();
                [type, nc] = strtok(tmp2);
                at.nelements = labelmesh.npoints;
                
                
                v = zeros(labelmesh.npoints,3);
                
                [num]=fscanf(fid,'%f');
                index=1;
                for i = 1:labelmesh.npoints
                    v(i,1)=   num(index);
                    index=index+1;
                    v(i,2)=   num(index);
                    index=index+1;
...
if (n_attributes>0)
    labelmesh.attributes = attributes;
end
fclose(fid);
end
